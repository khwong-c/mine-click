...
func PanicRecovery(cfg *config.Config, rend *render.Render, logger *slog.Logger) func(http.Handler) http.Handler {
			...
			defer func() {
				if rvr := recover(); rvr != nil {
					if err, ok := rvr.(error); ok {
						...
						handleErr(err, cfg, rend, logger, w, r)
					} else {
						logger.Error("U.N.Known Panic Recovered", "panic", rvr)
					}
				}
			}()
			next.ServeHTTP(w, r)
